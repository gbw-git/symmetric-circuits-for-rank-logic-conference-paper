...
$\vert \mathcal{A} \vert$ is the cardinality of its universe. Throughout this
paper we assume all algebraic structures are finite.

\subsection{First Order Logic and Counting}
...
quantity. We now briefly describe the structure of a formula in $\FPR[\tau]$.
Each variable is either a number or vertex variable, with vertex variables
assigned to elements of the universe and number variables assigned to natural
numbers. All atomic formulas in $\FP[\tau]$ are atomic formulas in $\FPR[\tau]$.
We say that $t$ is a \emph{number term} if $t$ is a number variable or if $t$ is
an application of the \emph{rank operator}, i.e.\ $t = [\rank (\vec{x}, \vec
{\nu} \leq \vec{t}, \vec{y},\vec{\mu} \leq \vec{s}, \pi \leq \eta). \phi]$,
where $\phi$ is a number term, $\vec{t}$ and $\vec{s}$ are tuples of number
...
The formulas of $\FPR$ are then formed by closing the set of atomic formulas
under the usual Boolean connectives, the first-order quantifiers, and the
fixed-point operator. We only allow bounded quantification for number terms.
Second-order variables, such as those that appear in a fixed-point application,
may have mixed-type.

We should note that there are two rank-logics in the literature. $\FPR$ is the
logic defined by Grad{\"e}l and Pakusa~\cite{GradelP15a}, rather than the
strictly less expressive rank logic introduced in~\cite{Dawar09logicswith}. We
let $\FOR[\tau]$ denote the set of all formulas in $\FPR[\tau]$ without an
application of the fixed-point operator. Let $\FOrk[\tau]$ denote
\emph{fixed-point logic with rank quantifiers} with respect to $\tau$. We define
for each prime $p$, and natural number $r$, a rank quantifier $\rank^r_p$, such
that $\rank^r_p\vec{x}\vec{y}. \phi$ is interpreted as $[\rank (\vec{x},
\vec{y}, \pi) . \phi] \leq t_r$, where $\pi$ is assigned to $p$ and $t_r$ is a
number term that evaluates to $r$. The arity of this quantifier is $\vert
\vec{x} \vert + \vert \vec{y} \vert$. Let $\mathcal{R}$ be the set of all such
quantifiers and $\FOrk[\tau]$ be the closure of $\FO[\tau]$ under $\mathcal{R}$.
For more details on rank quantifiers see~\cite{Dawar09logicswith}.
...
of a permutation of $V$, i.e.\ under the action of an isomorphism.
...
$\PT$-uniform family of transparent symmetric $(\RB, \rho)$-circuits that
compute a $q$-ary query.

From the Immerman-Vardi theorem~\cite{Immerman198686, Vardi:1982} and
Lemma~\ref{lem:transparent-unique}, there is an interpretation $\Phi$ that for
each $\rho$-structure $\mathcal{A}$ of size $n$, when $\Phi$ is interpreted in
$\mathcal{A}$ defines a symmetric rank-circuit with unique labels in the number
universe equivalent to $C_n$. We show that there is an $\FPR[\rho]$-formula
$Q(\vec{x})$ such that $\mathcal{A} \models Q[\vec{a}]$ if, and only if,
$C_n[\gamma \mathcal{A}](\Omega (\gamma \vec{a})) = 1$ for any bijection
$\gamma$. We construct $Q$ from an $\FPR$-definable procedure for evaluating a
circuit.
...
evaluation is entirely determined by the set $\EV_g := \{ \eta \in
U^{\underline{\consp(g)}} : \exists \gamma \in [n]^{\underline{U}}\text{ s.t. }
C_n[\gamma \mathcal{A}](g) = 1 \text{ and } \eta \sim \gamma^{-1}\}$. We show
that $\EV_g$ can be defined recursively, i.e.\ we define $\EV_g$ given $\{\EV_h
: h \in H_g \}$. If $g$ is a symmetric gate then we have a $\FPC$-definable
recursive construction of $\EV_g$ from~\cite{AndersonD17}. We assume $g$ is a
rank-gate and describe an $\FPR$-definable recursive procedure for defining
$\EV_g$.

The construction of $\EV_g$ in~\cite{AndersonD17} uses the fact that the
evaluation of a symmetric gate is determined by the number of inputs that
evaluate to true, and there is a bijection between between the orbit of a gate
and the assignment to the support. They use these results to define $\EV_g$
using a counting argument. The results that underlie this counting argument fail
for non-symmetric gates, and so we are forced to use a very different approach.

We show that for any $\eta \in U^{\underline{\consp(g)}}$ there is an
$\FPR$-definable matrix $M$ that has the same rank as $L^{\gamma
  \mathcal{A}}(g)$ for any $\gamma \in [n]^{\underline{U}}$. It follows that we
can check if $\eta \in \EV_g$ by applying the rank operator to $M$.
...
We now define the matrix $M$. We first define the index sets for $M$. Let
$R^{\min} := \{\min (\orb(\row(h))) : h \in H_g\}$ and $C^{\min} := \{ \min
(\orb (\column(h))) : h \in H_g\}$. Let $I := \{(i, \vec{x}): i \in R^{\min},
\vec{x} \in A_i\}$ and $J := \{(j, \vec{y}): j \in C^{\min}, \vec{y} \in A_j\}$.
...
We associate with each index $((i, \vec{x}), (j, \vec{y}))$ a gate $h$ and
$\vec{w}$ an assignment to the support of $h$ as follows. We can define $\sigma
\in \stab(g)$ such that $\vec{y} \sigma$ is compatible with both $\eta$ and
$\vec{x}$. Let $h = L(g)(i, \sigma j)$ and let $\vec{w} = (\vec{x} \vert \vec{y}
\sigma)$. We define the matrix $M : I \times J \ra \{0,1\}$ by $M((i , \vec{x}),
(j, \vec{y})) := \vec{w} \in \EV_h$.
...
$\gamma^{-1} \sim \eta$. Let $\Pi^{\gamma}_{f} \in \spstab{g}$ such that
...
establishes a correspondence between $A_h$ and the orbit of $h$. It follows that
$\vec{z} \in \EV_h$ if, and only if, $C_n[\gamma
\mathcal{A}](\Pi^{\gamma}_{\vec{z}}(h)) = 1$.

We idea is to use this correspondence to define a mapping from $M$ to $L^{\gamma
