...
% arity. In that case $C$ computes a constant function. For this reason, we
% always assume a circuit contains at least one relational gate with non-zero
% arity. Now, by assumption there exists a relational gate in $C$ such that some
% element of $[n]$ appears in the tuple labelling that gate. By symmetry it
% follows that every element of $[n]$ appears in a tuple labelling a relational
% gate in $C$. It follows that no two distinct elements of $\sym_n$ agree on all
% input gates, And so the homomorphism $h$ is injective. If $h$ is also
% surjective then we have that each element of $\sigma$ extends uniquely to an
% automorphism of the circuit. In this case we say that a circuit has
